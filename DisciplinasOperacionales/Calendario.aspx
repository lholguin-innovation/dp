<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Calendario.aspx.cs" Inherits="Calendario" %>
    ...
    $(document).ready(function () {  
                     $('#calendar').fullCalendar({
                                ...
                                events:  function(start, end, timezone , callback){
                                       $.ajax({
                                            url:"Calendario.aspx/getAgenda",
                                            type:"post",
                                            data:JSON.stringify({ 
                                                planta:$("#lblPlanta").text(), 
                                                nombre:$("#lblNombre").text(), 
                                                usuario: $("#lblUsuario").text(),
                                                mes:new Date($('#calendar').fullCalendar('getDate')).getUTCMonth()+1,
                                                anio:new Date($('#calendar').fullCalendar('getDate')).getUTCFullYear()
                                            }),
                                            contentType: "application/json; charset=utf-8",
                                            dataType: "json",
                                            success:function(r){  
                                                var events= r.lstData; 
                                                callback(events);   
                                            },
                                            error:function(err, h, x){
                                                failureCallback(err);
                                            }
                                        });
                                },
